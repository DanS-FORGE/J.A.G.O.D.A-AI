...
def _scan_dashboard_processes(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the J.A.G.O.D.A Desktop Electron app to protect its own
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by J.A.G.O.D.A Desktop (or become PPID-1
...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    ...
    # When the J.A.G.O.D.A Desktop Electron app spawns this dashboard as a
...
def _detect_concurrent_hermes_instances(
    ...
    The J.A.G.O.D.A Desktop Electron app spawns ``hermes.EXE`` as a backend child,

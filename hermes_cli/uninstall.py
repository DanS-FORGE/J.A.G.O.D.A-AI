    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# J.A.G.O.D.A" comment and following line (also
                # matches the pre-rebrand "# Hermes Agent" marker so older
                # installs still get cleaned up).
                if '# J.A.G.O.D.A' in line or '# Hermes Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def run_gui_uninstall(args):
    ...
    if not summary["gui_installed"]:
        print("No J.A.G.O.D.A Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The J.A.G.O.D.A Agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The J.A.G.O.D.A Agent at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The J.A.G.O.D.A Agent is still installed. Run 'hermes' to use the CLI,")

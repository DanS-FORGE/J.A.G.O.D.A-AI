...
Usage:
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native → J.A.G.O.D.A + Honcho
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '◉ J.A.G.O.D.A'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_uninstall(args):
    """Uninstall J.A.G.O.D.A (or just the Chat GUI with --gui)."""
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ J.A.G.O.D.A Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ J.A.G.O.D.A Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for J.A.G.O.D.A Desktop (Linux only, best-effort).
...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode:
        print("→ Launching J.A.G.O.D.A Desktop from source build...")
    ...
    print(f"→ Launching packaged J.A.G.O.D.A Desktop: {' '.join(launch_command)}")
...
def _quarantine_running_hermes_exe(
    ...
    handles (recovers in <1s), or the J.A.G.O.D.A Desktop backend child process
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running J.A.G.O.D.A Desktop / gateway / REPL).
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close J.A.G.O.D.A Desktop, exit other `hermes` REPLs, stop the "
...
def cmd_update(args):
    """Update J.A.G.O.D.A to the latest version.
...
def cmd_acp(args):
    """Launch J.A.G.O.D.A as an ACP server."""
    try:

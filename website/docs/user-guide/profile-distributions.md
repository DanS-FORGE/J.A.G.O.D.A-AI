...
A **profile distribution** packages a complete J.A.G.O.D.A — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.
...
Before distributions, sharing a J.A.G.O.D.A meant sending someone:

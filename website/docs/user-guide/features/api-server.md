...
Tool calls in the `output` array were already executed server-side by the J.A.G.O.D.A — they are replayed with `"status": "completed"` for structured tool UI, never as pending calls for the client to execute.

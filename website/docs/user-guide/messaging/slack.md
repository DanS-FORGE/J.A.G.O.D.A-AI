...
4. Enter an app name (e.g., "J.A.G.O.D.A") and select your workspace
5. Click **Create App**
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@J.A.G.O.D.A what time is it?`). In channels, Hermes replies in a thread attached to that message. |
...
Make sure the bot has been **invited to the channel** (`/invite @J.A.G.O.D.A`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @J.A.G.O.D.A` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @J.A.G.O.D.A` |
...
7. ✅ Bot was **invited** to the channel (`/invite @J.A.G.O.D.A`)

...
- **Uninstall Chat GUI only** — removes the desktop app and its data; the J.A.G.O.D.A, your config, and your chats stay. (Same as `hermes uninstall --gui`.)
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting J.A.G.O.D.A Desktop to a remote backend](./features/web-dashboard.md#connecting-hermes-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & J.A.G.O.D.A Desktop](../reference/environment-variables.md#web-dashboard--hermes-desktop).

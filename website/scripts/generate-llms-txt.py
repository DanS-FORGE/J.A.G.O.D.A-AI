...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# J.A.G.O.D.A Agent")
    lines.append("")
    lines.append(
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# J.A.G.O.D.A Agent — Full Documentation\n",
        (
            "This file is the entire J.A.G.O.D.A Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using J.A.G.O.D.A, Features, Messaging, Integrations, Guides, "

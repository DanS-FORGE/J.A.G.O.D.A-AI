...
// Electron identity anchor — feeds app.setName()/AppUserModelID, which in turn
// determines the OS userData folder (~/Library/Application Support/Hermes,
// %APPDATA%\Hermes) and Windows taskbar/jump-list grouping. Left as 'Hermes'
// deliberately: renaming it would silently move every user's config/sessions
// to a new folder on next launch and break `hermes_cli/gui_uninstall.py`'s
// on-disk discovery of existing installs. Not for display — see
// APP_DISPLAY_NAME for the user-visible J.A.G.O.D.A branding.
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'Hermes'
// User-visible name only (macOS menu bar, About panel, HUD window title) —
// decoupled from APP_NAME so branding can change without touching the
// identity anchor above.
const APP_DISPLAY_NAME = 'J.A.G.O.D.A'
const HUD_WINDOW_TITLE = `${APP_DISPLAY_NAME} HUD`
...
function resolveHermesBackend(backendArgs) {
  ...
  return {
    ...
    label: 'J.A.G.O.D.A Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
  ...
  if (IS_MAC) {
    template.push({
      ...
      submenu: [
        ...
        { role: 'services' },
        { type: 'separator' },
        // Explicit `label` overrides Electron's role-based auto-label (which
        // would otherwise read app.getName() === 'Hermes', the identity
        // anchor kept for userData/AppUserModelID compatibility — see
        // APP_NAME above).
...
async function startHermes() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('J.A.G.O.D.A Desktop is already running in another window.')
  }

...
async function locateHermes(ssh, remoteHermesPath) {
  ...
  const err: any = new Error(
    'Hermes is not installed on the remote host (could not find a `hermes` executable). ' +
      'Install it on the remote with:  curl -fsSL https://raw.githubusercontent.com/DanS-FORGE/J.A.G.O.D.A-AI/main/scripts/install.sh | sh  ' +
...
// e.g. "J.A.G.O.D.A v0.18.2 ..."), or '' on failure. Surfaces WHICH hermes a
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". J.A.G.O.D.A Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
async function connect(deps) {
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this J.A.G.O.D.A Desktop build (${lock.reason}). ` +

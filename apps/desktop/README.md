...
**The native desktop app for [J.A.G.O.D.A Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.
...
Prebuilt installers are built and distributed via [the J.A.G.O.D.A Desktop website.](https://hermes-agent.nousresearch.com/).
...
first launch it can install the J.A.G.O.D.A Agent runtime into `HERMES_HOME`
...
- **J.A.G.O.D.A Agent** runs as a headless `hermes serve` process and exposes the

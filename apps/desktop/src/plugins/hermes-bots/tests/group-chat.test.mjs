...
test('default profile speaks as J.A.G.O.D.A in room transcripts, not @default', () => {
  const gc = load(() => '(pass)')
  const line = gc.formatGroupChatLine({ from: { kind: 'member', name: 'default' }, text: 'hello room' }, 'builder')
  assert.equal(line, 'J.A.G.O.D.A: hello room')
  ...
  assert.equal(you, 'J.A.G.O.D.A (you): hi')
  const plain = gc.formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' }, 'research')
  assert.equal(plain, 'builder: yo')
})

test('speaker labels honor friendly identity: Bot Mode title, then display_name, never a stale J.A.G.O.D.A', () => {
  const gc = load(() => '(pass)')
  ...
  // "J.A.G.O.D.A is thinking…" in group rooms.
  ...
  // Untitled rows keep today's behavior: default → J.A.G.O.D.A, others verbatim.
  gc.$botMeta.set({})
  assert.equal(gc.groupSpeakerLabel('default'), 'J.A.G.O.D.A')

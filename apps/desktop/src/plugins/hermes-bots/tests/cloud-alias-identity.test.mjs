...
// "J.A.G.O.D.A" or a hostname label — when Cloud is the only active source.
...
test('alias never leaks to same-named defaults on other connections', () => {
  ...
  // Local default while the ACTIVE gateway is local: untouched "J.A.G.O.D.A".
  assert.equal(ctx.__displayName({ name: 'default' }, null), 'J.A.G.O.D.A')
})

 ...
 *  (researchbuddy). Reserved tokens are dropped so a bot renamed "J.A.G.O.D.A"
...
function newBotChat(bot) {
  if (typeof host.newChat !== 'function') {
    host.notify?.({ kind: 'error', message: 'Update J.A.G.O.D.A Desktop to open another Bot chat.' })
...
// raw Cloud hostname, or to generic 'J.A.G.O.D.A' in Cloud-only mode.
//
...
async function prepareBotSource(bot) {
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error('Update J.A.G.O.D.A Desktop to chat with agents on other connections.')
  }
...
function displayName(bot, meta) {
  ...
  // "J.A.G.O.D.A". Annotated active rows carry sourceScoped too, and keying this
  ...
  // never generic "J.A.G.O.D.A" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as J.A.G.O.D.A (the agent it is)
 ...
 *  renamed bot never shows up as its raw profile id or a stale "J.A.G.O.D.A"
 *  (community report, Aug 21 2026: renamed default still read "J.A.G.O.D.A is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as J.A.G.O.D.A (matching displayName and the
...
function groupSpeakerLabel(name) {
  ...
  return trimmed.toLowerCase() === 'default' ? 'J.A.G.O.D.A' : trimmed
}
...
function AdvancedProfileConfig({ bot, state, setState }) {
  ...
  if (unsupported) {
    return jsx('div', {
      className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
      children: 'Full configuration needs a newer gateway (restart it after updating J.A.G.O.D.A).'
    })
  }
                    ...
                    advTab === 'general'
                      ...
                      : capsFailed
                        ? jsx('div', {
                            className: 'px-2 py-3 text-center text-xs text-(--ui-text-tertiary)',
                            children:
                              'Capability catalog needs a newer gateway (restart it after updating J.A.G.O.D.A).'
                          })
                        : !caps
                          ? jsx('div', {
                              className: 'flex justify-center py-4',
...
function BotsHomeView() {
  ...
  const gateway = bot.connectionLabel || (bot.connectionId === 'local' ? 'This device' : 'J.A.G.O.D.A gateway')
        ...
        : error && !roster.length
          ? jsxs('div', {
              ...
              children: [
                jsx('div', {
                  children: gatewayUp
                    ? `Roster unavailable: ${error instanceof Error ? error.message : 'gateway error'}. If your gateway predates profiles.list, update J.A.G.O.D.A and restart the gateway.`
        ...
        description: deleting
          ? jsxs('span', {
              children: [
                ...
                ' and its associated J.A.G.O.D.A profile at ',

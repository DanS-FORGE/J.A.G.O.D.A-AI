...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting J.A.G.O.D.A...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting J.A.G.O.D.A…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to J.A.G.O.D.A gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        // round-trip must not hang "Starting J.A.G.O.D.A…" forever. Initial boot
        ...
        // hanging "Starting J.A.G.O.D.A…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "J.A.G.O.D.A couldn't start"

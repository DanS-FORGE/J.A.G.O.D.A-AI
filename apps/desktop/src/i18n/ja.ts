...
export const ja = defineLocale({
  ...
  notifications: {
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'J.A.G.O.D.A が応答を待っています。',
      turnDoneTitle: 'J.A.G.O.D.A が完了しました',
      turnDoneBody: '',
  ...
  settings: {
    ...
    notifications: {
      ...
      kinds: {
        ...
        input: {
          label: '入力が必要',
          description: 'J.A.G.O.D.A が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
    ...
    appearance: {
      ...
      reactionsDesc:
        'iMessage風の絵文字タップバック — メッセージにリアクションでき、J.A.G.O.D.Aもあなたのメッセージにリアクションします。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、J.A.G.O.D.A の状態に反応します。',
        restartHint:
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'J.A.G.O.D.A がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'J.A.G.O.D.A が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'J.A.G.O.D.A が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを J.A.G.O.D.A がどう扱うかを設定します。',
    ...
    gateway: {
      ...
      sshHermesPathTitle: 'J.A.G.O.D.A パス（任意）',
      ...
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— J.A.G.O.D.A を検出`,
      ...
      sshErrNotInstalled:
        'リモートホストに J.A.G.O.D.A がインストールされていません。リモートでインストールする（curl -fsSL https://hermes-agent.nousresearch.com/install.sh | sh）か、J.A.G.O.D.A パスを設定してください。',
      sshErrPlatform:
  ...
  starmap: {
    ...
    emptyDesc: 'J.A.G.O.D.A がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    generatePet: {
      ...
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら J.A.G.O.D.A が通知します。',
  ...
  cron: {
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。J.A.G.O.D.A が実行して、選択した宛先に結果を送信します。',
  ...
  composer: {
    ...
    attachUrlDesc: 'J.A.G.O.D.A がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'J.A.G.O.D.A にコミットと PR を任せる',
  ...
  install: {
    ...
    settingUpTitle: 'J.A.G.O.D.A Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の J.A.G.O.D.A CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の J.A.G.O.D.A インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
  ...
  onboarding: {
    headerTitle: 'J.A.G.O.D.A Agent のセットアップをしましょう',
  ...
  preview: {
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'J.A.G.O.D.A を再起動中...',
      askRestart: 'J.A.G.O.D.A にサーバーの再起動を依頼',
      lookingRestart: taskId => `J.A.G.O.D.A は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'J.A.G.O.D.A はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、J.A.G.O.D.A にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `J.A.G.O.D.A がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'J.A.G.O.D.A がサーバーを再起動できませんでした。',
      stillWorking:
        'J.A.G.O.D.A はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'J.A.G.O.D.A が応答を読み込み中',
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.hermes/config.yaml) に追加されます。J.A.G.O.D.A はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'J.A.G.O.D.A は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'J.A.G.O.D.A は続行するための認証情報が必要です。',

...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'J.A.G.O.D.A Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'J.A.G.O.D.A docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('J.A.G.O.D.A Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'J.A.G.O.D.A docs', url: 'https://example.com/docs' }
    ])
  })
})

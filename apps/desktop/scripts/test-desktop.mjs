...
// no longer bundles the J.A.G.O.D.A Agent Python payload (that's fetched at first
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `J.A.G.O.D.A-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `J.A.G.O.D.A-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `J.A.G.O.D.A-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'J.A.G.O.D.A-${version}-${os}-${arch}.${ext}'
...
//   - The J.A.G.O.D.A Agent Python payload is NOT shipped (it's fetched at first

#!/usr/bin/env node
// set-exe-identity.mjs — stamp the J.A.G.O.D.A icon + version metadata onto
// the built Hermes.exe (filename unchanged — see executableName in
// package.json) using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'J.A.G.O.D.A',
      FileDescription: 'J.A.G.O.D.A',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — J.A.G.O.D.A icon + identity stamped')
}

...
describe('DEFAULT_THEME', () => {
  ...
  it('has color palette', async () => {
    ...
    expect(DEFAULT_THEME.color.primary).toBe('#E8EDF5')
...
describe('derived tone ladder', () => {
  it('reproduces the J.A.G.O.D.A hand-verified tones from seeds (reverse-engineered knobs)', async () => {
    // The ladder's knobs were grid-search fitted so the MATH lands on
    // hand-tuned literals. Contract: every derived tone stays within a-few-
    // RGB-units of the reference (imperceptible), so knob edits that drift
    // the identity fail here instead of shipping as vibes. Rebaselined for
    // the J.A.G.O.D.A chrome/cyan/red identity (was Hermes gold) — the ladder
    // math is unchanged, only the seed-derived reference literals are new.
    const dark = await importThemeWithCleanEnv()
    const light = await importThemeWithEnv({ HERMES_TUI_BACKGROUND: '#ffffff' })

    const cases: Array<[string, string, string]> = [
      [dark.DARK_THEME.color.muted, '#3588c4', 'dark muted'],
      [dark.DARK_THEME.color.label, '#3891d1', 'dark label'],
      [dark.DARK_THEME.color.statusFg, '#b4b4b4', 'dark statusFg'],
      [dark.DARK_THEME.color.completionBg, '#141A24', 'dark surface'],
      [dark.DARK_THEME.color.completionCurrentBg, '#1C2E44', 'dark chip'],
      [dark.DARK_THEME.color.selectionBg, '#1F3350', 'dark selection'],
      [light.LIGHT_THEME.color.muted, '#0f6eac', 'light muted'],
      [light.LIGHT_THEME.color.statusFg, '#676767', 'light statusFg'],
      [light.LIGHT_THEME.color.completionBg, '#f5f8fb', 'light surface'],
      [light.LIGHT_THEME.color.completionCurrentBg, '#bbd6e8', 'light chip'],
      [light.LIGHT_THEME.color.selectionBg, '#d2e2ed', 'light selection']
    ]
